...
# ---- ЗАГРУЗКА БАЗЫ СЛОВ ИЗ ВНЕШНЕГО ФАЙЛА ----
try:
    with open(resource_path("guess_word_words_list.txt"), "r", encoding="utf-8") as file:
        # Очистка
        words = list(dict.fromkeys([line.strip().lower() for line in file if line.strip()]))
    print(f"Успешно загружено уникальных слов: {len(words)}")
except FileNotFoundError:
    # В случае потери файла
    words = ["каток", "оплот", "слово", "книга", "ручка"]
    print("Файл guess_word_words_list.txt не найден! Загружен резервный список.")
...
def save_game_progress():
    ...
    print("Прогресс успешно сохранены в скрытый системный файл!")
...
def load_game_progress():
    ...
    if not os.path.exists(SAVE_FILE_PATH):
        print("Файл не найден. Автоматически разворачиваем структуру сохранения...")
        save_game_progress()
        return
    ...
    try:
        ...
        current_win_streak = save_data.get("current_win_streak", 0)
        max_win_streak = save_data.get("max_win_streak", 0)
        total_completed_quests = save_data.get("total_completed_quests", 0)
        last_update_day = save_data.get("last_update_day", -1)
        active_quests = save_data.get("active_quests", {})
        
        unlocked_themes = save_data.get("unlocked_themes", {})
        for k, is_unlocked in unlocked_themes.items():
            if k in color_themes:
                color_themes[k]["unlocked"] = is_unlocked
                
        unlocked_achivements = save_data.get("unlocked_achivements", {})
        for k, ach_info in unlocked_achivements.items():
            if k in achivements:
                achivements[k]["got"] = ach_info.get("got", False)
                achivements[k]["date"] = ach_info.get("date", "")
                
        active_theme = save_data.get("active_theme_name", "classic")
        if active_theme in color_themes:
            choose_theme(active_theme)
            
        print("Системный файл сохранения успешно прочитан, квесты восстановлены!")
        
    except json.JSONDecodeError:
        print("Файл поврежден. Автоматически разворачиваем структуру сохранения...")
        save_game_progress()
